...
pub fn seed_stream_key(state: &Arc<AppState>, name: &str) -> (String, String) {
    ...
    let key_token: String = (0..24).map(|_| format!("{:02x}", rand::random::<u8>())).collect();
    conn.execute(
...
pub fn seed_room(state: &Arc<AppState>, name: &str, slug: &str) -> String {
    let conn = state.db.get().unwrap();
    let id = uuid::Uuid::new_v4().to_string();
    let presenter_key: String = (0..16).map(|_| format!("{:02x}", rand::random::<u8>())).collect();
    conn.execute(
...
#[allow(dead_code)]
pub fn seed_room_full(state: &Arc<AppState>, name: &str, slug: &str, status: &str, waiting_room: bool, stream_key_id: Option<&str>) -> String {
    let conn = state.db.get().unwrap();
    let id = uuid::Uuid::new_v4().to_string();
    let presenter_key: String = (0..16).map(|_| format!("{:02x}", rand::random::<u8>())).collect();
...
#[allow(dead_code)]
pub fn seed_participant(state: &Arc<AppState>, room_id: &str, name: &str, role: &str, admitted: bool, kicked: bool) -> (String, String) {
    let conn = state.db.get().unwrap();
    let id = uuid::Uuid::new_v4().to_string();
...
pub fn get_room_presenter_key(state: &Arc<AppState>, room_id: &str) -> String {
    let conn = state.db.get().unwrap();
    conn.query_row("SELECT presenter_key FROM rooms WHERE id = ?1", rusqlite::params![room_id], |row| row.get(0)).unwrap()
}
